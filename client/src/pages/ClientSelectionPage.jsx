...
const ClientSelectionPage = ({ onLogout }) => {
    ...
    // );
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-[#181737] text-gray-200 p-4">
             <div className="absolute top-4 right-4">
                <button
                    onClick={onLogout}
            ...
            <div className="w-full max-w-lg p-8 bg-gradient-to-br from-[#3E39A1] to-[#15F39E] rounded-lg shadow-lg text-center">
                <Users className="mx-auto h-16 w-16 text-[#15F39E] mb-6" />
                <h1 className="text-3xl font-bold text-white mb-6">Select Client Dashboard</h1>
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                        {clients.map(client => (
                            <button
                                key={client.id}
                                onClick={() => handleClientSelect(client.id)}
                                className="w-full px-4 py-3 text-left bg-[#15F39E] rounded-md hover:bg-[#10b575] focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-150 ease-in-out"
                            >
                                {/* --- FIX: Display client.name instead of ID --- */}
                                <span className="font-medium text-black">{client.name || Client (`ID: ${client.id}`)}</span>
                                <span className="block text-sm text-gray-800">{client.email}</span>
                            </button>
                        ))}
                    </div>
                ) : (
                    !error && <p className="text-gray-800">No clients assigned or available.</p>
                )}
            </div>
        </div>
    );
